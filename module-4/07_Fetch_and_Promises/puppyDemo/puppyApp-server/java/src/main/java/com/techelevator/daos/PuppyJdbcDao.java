...
public class PuppyJdbcDao implements PuppyDao {
    ...
    public void savePuppy(Puppy puppyToSave) {
        String sqlAddPuppy = "INSERT INTO puppies " +
            "(name, weight, gender, paper_trained) VALUES " +
            "(?, ?, ?, ?)";
        template.update(sqlAddPuppy,
            puppyToSave.getName(),
            puppyToSave.getWeight(),
            puppyToSave.getGender(),
    ...
    private Puppy mapResultToPuppy(SqlRowSet result) {
        Puppy retrievedPuppy = new Puppy(
            ...
            result.getBoolean("paper_trained")
        );
