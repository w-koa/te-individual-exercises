...
SELECT continent, sum(surfacearea) as total_area
FROM country
GROUP BY continent
...
SELECT gnp - gnpold as gnp_difference, ABS(gnp - gnpold) as absolute_difference, name
FROM country
...
SELECT AVG(lifeexpectancy) as average_life, MIN(lifeexpectancy) as shortest_life, MAX(lifeexpectancy) as longest_life, continent
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
