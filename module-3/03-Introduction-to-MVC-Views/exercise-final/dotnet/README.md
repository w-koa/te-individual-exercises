...
Modify the `FizzBuzz` view. The format for the URL will be `http://localhost:xxxxx/home/fizzbuzz?number=20`. The number to go up to is available via the `ViewData["number"]` property.
...
Modify the `Fibonacci` view that displays the numbers of the Fibonacci sequence up to N, where N is the value of `max`. The format for the URL will be: `http://localhost:xxxxx/home/fibonacci?max=50`. The number to go up to is available view the `ViewData["max"]` property.
