...
public class Lecture {
	...
	public static void main (String [] args) {
		...
		boolean result = obj.returnCounterFromLoop();
    ...
    */
    public int returnLastElement() {
        int[] portNumbers = { 80, 8080, 443 };
        return portNumbers[portNumbers.length - 1]; // return portNumbers[2]; 
    }
    ...
    */
    public int returnLastElementOfParam(int[] passedInArray) {
        return passedInArray[passedInArray.length - 1];  // how many elements were in passedInArray?  No idea!
    }
    ...
    */
    public int returnVariableFromBlock(int number) { // #1 block
    	int result;
        { // A new block with scoped variables  -- #2 block

            result = number * 5;

        } // the result variable disappears here -- end #2 block

        return result; // We want to return result here. How? -- end #1 block
    }
